...
where $$x_j= \Delta - \xi_j$$ and $$e_{m}$$ is the elemtary symmetric polynomial. Thus, we may consider the following sequence

$$
\xi_i(k+1) = \left(\xi_i^L \prod_{j=1}^{N}(\xi_j^{-1} + \xi_i  - \Delta \xi_i\xi_j^{-1}) + (-1)^N \sum_{m\neq 1}^{N} (-1)^{N-m} e_{N-m}(x_1, \dots, x_n) \xi_i^m\right)/e_{N-1}(x_1, \dots, x_N)
$$
