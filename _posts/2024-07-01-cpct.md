...
&[\prod_{j \neq i} \Delta - \tfrac{w_j}{v_j}: \Delta + (-1)^N \left((\tfrac{v_i}{w_i})^L \prod_{j=1}^{N}(\tfrac{w_j}{v_j} + \tfrac{v_i}{w_i}  - \Delta \tfrac{v_i w_j}{w_i v_j}) + (-1)^N \prod_{j=1}^N(v_i /w_i - (\Delta - w_j/v_i)) - \prod_{j =1}^N (\Delta - \tfrac{w_j}{v_j}) \right)]
\end{align}
$$
